...
export const fetchDurations = async (names: string[], w: number, h: number, fetchElt: HTMLElement) => {
  ...
  await Promise.all(names.map(async (name) => {
    ...
    fetchElt.innerText = `fetched durations ${Object.keys(durations).length}/${names.length}`;
  }));
  return durations;
};

export const fetchShiftss = async (names: string[], w: number, h: number, fetchElt: HTMLElement) => {
  ...
  await Promise.all(names.map(async (name) => {
    ...
    // eslint-disable-next-line no-param-reassign
    fetchElt.innerHTML = `fetched shifts ${Object.keys(shifts).length}/${names.length}`;
  }));
  return shifts;
};
